...
const DURATION_OPTIONS = [{ id: 'w1', label: '1주 이내' }, { id: 'w2', label: '2주' }, { id: 'm1', label: '1개월' }, { id: 'm2', label: '2개월 이상' }];
const CLIENT_OPTIONS = [{ id: 'startup', label: '스타트업' }, { id: 'smb', label: '소상공인' }, { id: 'solo', label: '개인사업자' }, { id: 'midsize', label: '중소기업' }, { id: 'corporate', label: '대기업' }, { id: 'public', label: '공공기관' }];
const BUDGET_OPTIONS = [{ id: 'b1', label: '30만원 이하' }, { id: 'b2', label: '30~100만원' }, { id: 'b3', label: '100~300만원' }, { id: 'b4', label: '300만원 이상' }];
const STYLE_OPTIONS = [
...
export default function BriefPage() {
  const [view, setView] = React.useState<'settings' | 'brief' | 'process'>('settings');
  const [form, setForm] = React.useState<BriefForm>({ field: null, difficulty: null, duration: null, client: null, budget: null, styles: [], refUrl: '', avoid: '' });
  const [generating, setGenerating] = React.useState(false);

  const update = <K extends keyof BriefForm>(key: K, value: BriefForm[K]) => setForm((f) => ({ ...f, [key]: value }));
  ...
  const handleSubmit = () => {
    setGenerating(true);
    setTimeout(() => { setGenerating(false); setView('brief'); window.scrollTo({ top: 0, behavior: 'instant' }); }, 1400);
  };
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'saturate(180%) blur(8px)', borderBottom: '1px solid var(--ink-200)', padding: '14px 36px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button type="button" style={{ background: 'transparent', border: 'none', color: 'var(--ink-600)', fontSize: 13, fontWeight: 500, padding: '8px 12px', borderRadius: 8, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            <Icon.Help style={{ width: 16, height: 16 }} /> 도움말
          </button>
          <div style={{ width: 32, height: 32, borderRadius: 999, background: 'linear-gradient(135deg,#FCE7F3,#DDD6FE)', color: 'var(--indigo-700)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, border: '1.5px solid var(--white)', boxShadow: 'var(--shadow-xs)' }}>민</div>
        </div>
      </header>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18, minWidth: 0 }}>
          ...
          >
            ...
            <Field label="참고 브랜드 / 사이트" optionalText="선택">
              <TextInput value={form.refUrl} onChange={(v) => update('refUrl', v)} placeholder="예: https://www.toss.im" leftIcon={Icon.Link} />
            </Field>
            <Field label="피해야 할 스타일" optionalText="선택" helper={<HelperHint>예: &quot;유치한 디자인&quot;, &quot;지나치게 화려한 그래픽&quot; 등</HelperHint>}>
              <textarea value={form.avoid} onChange={(e) => update('avoid', e.target.value)} placeholder="피해야 할 스타일을 자유롭게 적어주세요" style={{ width: '100%', minHeight: 72, padding: '12px 14px', border: '1.5px solid var(--ink-200)', borderRadius: 'var(--radius)', outline: 'none', fontSize: 14, resize: 'vertical', background: 'var(--white)', color: 'var(--ink-900)', fontFamily: 'inherit' }}
      ...
      <div style={{ position: 'sticky', bottom: 0, left: 0, right: 0, background: 'rgba(255,255,255,0.92)', backdropFilter: 'saturate(180%) blur(8px)', borderTop: '1px solid var(--ink-200)', padding: '14px 36px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button type="button" style={{ background: 'var(--white)', border: '1px solid var(--ink-200)', color: 'var(--ink-700)', fontSize: 14, fontWeight: 600, padding: '12px 18px', borderRadius: 'var(--radius)' }}>임시 저장</button>
          <button type="button" disabled={!canSubmit || generating} onClick={handleSubmit} style={{ background: canSubmit ? 'var(--indigo-600)' : 'var(--ink-200)', border: 'none', color: canSubmit ? 'var(--white)' : 'var(--ink-400)', fontSize: 14.5, fontWeight: 700, padding: '12px 22px', borderRadius: 'var(--radius)', display: 'inline-flex', alignItems: 'center', gap: 8, cursor: canSubmit ? 'pointer' : 'not-allowed', boxShadow: canSubmit ? 'var(--shadow-indigo)' : 'none', transition: 'all 140ms ease', letterSpacing: '-0.01em' }}>
            {generating ? (
              <><div style={{ width: 14, height: 14, borderRadius: 999, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: 'var(--white)', animation: 'spin 0.7s linear infinite' }} />생성 중…</>
            ) : (
